                ...
                {submitted && (
                    <div className="shadow-lg rounded-lg p-6 mb-4 bg-white flex justify-between">
                        ...
                        <div className="flex space-x-4 font-semibold">
                            <p><span className="inline-block w-3 h-3 bg-green-500 mr-2"></span>Correct: {score.Correct}</p>
                            <p><span className="inline-block w-3 h-3 bg-red-500 mr-2"></span>Incorrect: {score.Incorrect}</p>
                            <p><span className="inline-block w-3 h-3 bg-yellow-500 mr-2"></span>Skipped: {score.Skipped}</p>
                        </div>
                    </div>
                )}
                {questions.map((question) => (
                    <div key={question._id} className="flex flex-col shadow-lg rounded-lg p-6 relative">
                        <h3 className="mb-2 text-xl font-bold mr-5">{question.question}</h3>
                        {submitted && (
                            <div className={`absolute top-0 right-0 p-2 text-white font-bold rounded-bl-lg ${
                                questionStatus[question._id] === "Correct"
                                    ? "bg-green-400"
                                    : questionStatus[question._id] === "Incorrect"
                                    ? "bg-red-400"
                        ...
                        <div className="flex flex-col gap-2">
                            {question.options.map((option) => (
                                <div key={option} className={`bg-gray-100 ${
                                    !submitted
                                        ...
                                        ? "bg-green-300"
                                        : "bg-gray-100"
                                } shadow-md rounded-lg p-4 cursor-pointer`} onClick={() => handleOptionChange(question._id, option)}>
                                    <p className="text-gray-800 font-semibold text-left">{option}</p>
                                </div>
                            ))}
                        </div>
                        {!submitted && (
                            <button className="mt-2 bg-blue-500 text-white py-1 px-3 rounded hover:bg-blue-700" onClick={() => handleOptionChange(question._id, null)}>
                                Clear Selection
                            </button>
                        )}
                    </div>
                ))}
                <button onClick={handleSubmit} disabled={submitted} className={`mt-4 px-4 py-2 bg-gray-500 text-white rounded hover:bg-green-400 ${
